...
import {
  signInAdminAction,
  signOutAdminAction,
  updateMunicipalityHomeMapAction,
} from "./actions";
      ...
      <section className="mx-auto flex w-full max-w-5xl flex-col gap-4 rounded-[2rem] border border-slate-900/10 bg-white/92 p-4 shadow-[0_20px_80px_rgba(15,23,42,0.08)] backdrop-blur sm:p-6">
        ...
        {!configured ? (
          <div className="rounded-[1.4rem] border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-900">
            {text.configuredMissing}
          </div>
        ) : null}

        {configured && !authenticated ? (
          <div className="max-w-xl rounded-[1.6rem] border border-slate-900/10 bg-slate-50/90 p-5">
            <h2 className="text-lg font-semibold text-slate-900">{text.loginTitle}</h2>
            <p className="mt-2 text-sm leading-6 text-slate-600">{text.loginBody}</p>
            <form action={signInAdminAction} className="mt-4 flex flex-col gap-3 sm:flex-row">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-300 bg-white px-3 py-3 text-sm text-slate-900 outline-none transition focus:border-teal-500"
              />
              <button
                type="submit"
